<?php
    ...
    // Check for Username Password match and Login.
		...
		if (isset($_POST['usern']) && isset($_POST['passw']) && !empty($_POST['usern']) && !empty($_POST['passw'])){
			...
			$rowcount=mysqli_num_rows($result);
			if($rowcount==0) {
				header("location: front.php?lmsg=1");
				exit();
			}
			if($rowcount==1) {
			...
			header("location: front.php?lmsg=2");
			exit();
		}
	}
    ...
    <div class="menu">
      ...
      <div class="icon-close">
		...
		<div class = "fupload-form">
			...
			?></i>
			<form action="login.php?op=1" method="post" enctype="multipart/form-data">
				<fieldset>
		...
		<div class = "fdelete-form">
			...
			?></i>
			<form action="login.php?op=2" method="post">
				<fieldset>
		...
		<div class="passchange-form">
			...
			?></i>
			<form method = "POST" action="login.php?op=3">
				<fieldset>
